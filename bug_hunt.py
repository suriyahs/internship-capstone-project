...
def intro():
    print("🐞 Welcome to CODE QUEST: The Bug Hunt!")
    print("You're a brave coder on a mission to catch coding bugs in the wild.")
...
def play_game(name):
    ...
    while attempts > 0 and bugs_caught < NUM_BUGS:
        ...
        if choice in bug_positions:
            ...
            if random.random() < 0.3:
                ...
                attempts -= 1  # Extra penalty
            else:
                print("😐 Just leaves... no bugs here.")

        attempts -= 1  # Normal attempt usage
    ...
    if bugs_caught == NUM_BUGS:
        print(f"\n🏆 Amazing, {name}! You caught all the bugs and squashed the code chaos!")
    elif bugs_caught > 0:
...
def main():
    ...
    while True:
        ...
        if replay != 'y':
            break
    print("\n👋 Thanks for playing Code Quest! Keep coding, and keep hunting those bugs!")
